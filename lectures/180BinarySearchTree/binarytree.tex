...
\begin{codebox}
  ...
  \li \Return $y$
\end{codebox}
\includegraphics[width=0.4\textwidth]{Fig-12-2.pdf}
...
\end{codebox}
\end{minipage}
\hspace{-1cm}\includegraphics[width=0.5\textwidth]{Fig-12-3.pdf}

\sect{Deletion}

To delete node $z$ from tree $T$:
\begin{enumerate}
  \ii If $z$ has no children, just remove it.
  \ii If $z$ has just one child, then make that child take $z$'s
  position in the tree, dragging the child's subtrees along.
  \ii If $z$ has two children, then
  \bi
  ...
  \bi
  \ii This case is tricky when $y$ is $z$'s right child.
  \ei
  \ei
\end{enumerate}


\includegraphics[height=\textheight]{Fig-12-4.pdf}

\sect{Transplant}

\bi
\ii \textsc{Transplant}$(T,u,v)$ replaces the subtree rooted at $u$
...
\bi
\ii $a=a_0a_1\ldots a_p$ is \textbf{lexicolgraphically less than}
$b=b_0b_1\ldots b_q$: 
  \begin{enumerate}
  \item
...
\ii A set $S$ of bit strings can be sorted lexicographically
